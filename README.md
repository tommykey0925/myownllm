...
| パラメータ | デフォルト値 | 説明 |
|---|---|---|
| `n_embd` | 128 | 埋め込みベクトルの次元数。大きいほど表現力が増すがメモリ・計算コストも増加 |
| `n_head` | 4 | マルチヘッド注意のヘッド数。`n_embd` の約数である必要がある |
| `n_layer` | 4 | Transformerブロックの積み重ね数。深いほど複雑なパターンを学習可能 |
...
| `max_iters` | 5000 | 学習ステップ数 |
| `eval_interval` | 500 | 検証損失を計算する間隔 (ステップ数) |
